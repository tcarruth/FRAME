...
VOI_MSC<-function(MSEobj,LTL=F,lev=80, ncomp = 10, nbins = 5,maxrow = 8,yrs=46:50){
  ...
  for(mp in 1:nMPs){
    Ut[, mp]=as.integer(MSC_Usim(MSEobj, MPno=mp,LTL=F, lev=80)>1, yrs=yrs)
  }
  ...
  # set up the plot
  layout(topcorner(nrow,ncol),widths=rep(c(1,1),ncol),heights=rep(c(1.6,1),nrow))
  ...
  for(mp in 1:nMPs){
    ...
    barplot(Obss[mp,ord],names.arg=Obsnam[ord],las=2,border=NA,ylim=obslim,col=colsse[colno])
    ...
    mtext(MSEobj@MPs[mp],3,adj=0.05,line=0.25,font=2)
    
    linefunc(x1=0.75,x2=ncomp*0.5,y1=Obss[mp,ord[1]], y2=0.75*obslim[2],col=colsse[colno[1]],gap=0.15)
   
    
    mids<-(Obsp[,ord[1]][1:nbins]+Obsp[,ord[1]][1+(1:nbins)])/2
    barplot(Obsv[mp,ord[1],],names.arg=round(mids,2),col=colsse[colno[1]],border=NA)
  ...
  mtext("Certification Uncertainty (StDev of probabilties across sampled variables)",2,line=0.9,outer=T,font=2)
