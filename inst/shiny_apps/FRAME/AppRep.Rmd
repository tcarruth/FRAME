...
```{r Ptab,echo=F,warning=F,error=F,message=F}
 ...
 Ptab_formatted(params$Ptab2_app,burnin=params$burnin,cols=params$MPcols,thresh=params$thresh)
 Thresh_tab(params$thresh)
...
```{r MSC_PMs,echo=F,warning=F,error=F,message=F,fig.width=11,fig.height=11}
  ...
  MSC_PMs(params$MSEobj_app,params$MSEobj_reb_app,MPcols=params$MPcols)
...
```{r pencil,echo=F,warning=F,error=F,message=F,results='asis',fig.width=12, fig.height=9}
 ...
 nMPs<-length(params$MSEobj_app@MPs)
 panelsperplot<-1
 nplots<-ceiling(nMPs/panelsperplot)
 plots<-split(1:nMPs, ceiling(seq_along(1:nMPs)/panelsperplot))
 
 for(i in 1:length(plots)){
   MSEobj2<-Sub(params$MSEobj_app,MPs=params$MSEobj_app@MPs[unlist(plots[i])])
...
```{r pencil2,echo=F,warning=F,error=F,message=F,results='asis',fig.width=12, fig.height=9}
 ...
 for(i in 1:length(plots)){
   MSEobj2<-Sub(params$MSEobj_app,MPs=params$MSEobj_app@MPs[unlist(plots[i])])
   Pplot4(MSEobj2,MPcols=params$MPcols[plots[[i]]],maxcol=1,maxrow=2)
 }
...
```{r rebuild,echo=F,warning=F,error=F,message=F,results='asis',fig.width=12, fig.height=9}
 ...
 nMPs<-length(params$MSEobj_reb_app@MPs)
 panelsperplot<-8
 nplots<-ceiling(nMPs/panelsperplot)
 plots<-split(1:nMPs, ceiling(seq_along(1:nMPs)/panelsperplot))
 
 for(i in 1:length(plots)){
   MSEobj2<-Sub(params$MSEobj_reb_app,MPs=params$MSEobj_reb_app@MPs[unlist(plots[i])])
...
```{r uncertain,echo=F,warning=F,error=F,message=F,results='asis',fig.width=12, fig.height=9}
 ...
 MSC_uncertain(params$MSEobj_app,MPcols=params$MPcols,maxMPs=params$MSEobj_app@nMPs, LTL=F,inc_thresh = F,burnin=params$burnin)
...
VOI_MSC(params$MSEobj_app,MPcols=params$MPcols)
