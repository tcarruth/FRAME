...
# Latest additions to FRAME 

## Version 0.93 (alpha)

* Options has a dedicated tab
* UI font / style overhaul
* Custom fonts now readily available for all text types
* UI restructured
* FIP and Risk Evaluation options properly greyed out

## Version 0.92 (alpha)

* Reactive panel system overhauled to improve reliability
* Separated discard rate and post-release mortality rate
* Ancillary indicator demonstration added
...
* Two additional modes of analysis: 'Fishery Improvement Project' and 'Risk Evaluation'
* Uploading of DLMtool / MSEtool operating models
* Uploading of custom DLMtool / MSEtool source code (management procedures / MSE controls / performance metrics)
* Dynamic evaluation of data requirements - ie proper evaluation of whether 'recent' data are sufficient

# Introduction

FRAME is a fishery risk assessment tool that links a streamlined user interface to an advanced operating model, using management strategy evaluation to identify sustainable fishery management options. The Application includes live help and links to external documentation. However this document is intended to provide additional information and guidance for FRAME users beyond the help provided by the online App

The current version of FRAME includes a total of 21 questions: 
* 14 focus on the nature of the stock and fishery dynamics, 
* 3 relate to the management system and 
* 4 describe the types of data that are available and their quality. 

FRAME allows for a high degree of uncertainty over any of these attributes. By default the questions are answered with the maximum level of uncertainty and assume bad quality of data. At any time the user can press 'Calculate' and the MSE will be calculated, even if the user has not prescribed any additional certainty. 

Questions are presented in order of importance (in general across many management procedures, although importance varies greatly among management procedures).

At the heart of FRAME is OMx, a rapid and flexible operating model developed for the data-limited methods toolkit (DLMtool, Carruthers and Hordyk 2018) and the management strategy evaluation toolkit (MSEtool, Carruthers et al. 2018).

The application is currently in Beta testing, all feedback can be submitted to <t.carruthers@oceans.ubc.ca>
<br><br>
...
Analysis Type: The Demo mode runs a small diverse range of management procedures for a small number of simulations to demonstrate the features of the FRAME application; The Risk Evaluation mode provides generic risk assessment outputs over a longer projected time period of 100 years; The FIP (Fishery Improvement Project) mode provides MSC-specific guidance to help fisheries progress towards certification: The Certification mode generates a standard calculation and reporting of quantities relevant to MSC certification.

Users can also determine the total number of simulations, projected years and the management update interval (years between management recommendations in the projection)

Other options include selecting a management procedure for Ancillary Indicator analysis, the loading of custom DLMtool/MSEtool code (MPs, performance metrics and MSE controls)
Users can also choose to ignor reference management procdures (e.g. zero catches, fishing at FMSY) and also activate parallel computation (which is much faster but there is no MSE progress bar).
