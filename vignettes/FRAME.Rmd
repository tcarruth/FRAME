...
The first is the Streamlined mode that is less complex and uses the quantitative questionnaire to parameterize a operating model for risk assessment of multiple MPs. The second, the Advanced mode includes tools for conditioning operating models on data, doing detailed MSE analysis of a single selected MP and also detecting 'exceptional circumstances' via auxillary indicator data. 
