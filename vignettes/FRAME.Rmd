...
* App redesigned to Streamlined and Advanced modes
* Risk assessment vs MSE now happens in Evaluation model with the Users choice of MPs
* MSE convergence diagnostics in Evaluation mode
* Management advice can now be calculated in-app
* Bug ricky detected in Application mode now fixed (need to run FMSYref in addition to chosen MP to calculate relative yield)
...
FRAME is fishery risk assessment software that links a streamlined user interface to the advanced operating model of DLMtool and MSEtool, using management strategy evaluation to identify sustainable fishery management procedures (MPs).
...
If data are loaded in step A2, it is possible to calculate management advice for all feasible MPs. MPs differ in the type of advice they specify providing TACs (total allowable catches), TAE (total allowable effort as a fraction of todays effort), size limits and area closures. 
